...
module NavigationHelpers
  ...
  def path_to(page_name)
    ...
    when /^the edit page for "(.+)"$/ then
      movie_path "#{Movie.find_by_title($1).id}/edit"
    when /^the details page for "(.+)"$/ then
      movie_path Movie.find_by_title($1).id.to_s
    when /^the Similar Movies page for "(.+)"$/ then
      movie_path "#{Movie.find_by_title($1).id}/similar"
    when /^the Search Results page$/ then
      search_tmdb_path
